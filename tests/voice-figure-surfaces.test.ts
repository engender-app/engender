...
describe('the voice figure', () => {
  ...
  it('cannot draw a cited band without its source and its caveat', () => {
    ...
    /* The caption renders wherever bands do, and bands are drawn on every
       figure there is. There were briefly two ways to silence it and this
       test asserted the expression containing both, so it ratified the
       second instead of catching it: `compact` drew the washes with no
       gutter, no legend, no source and no caveat. One way now. */
    expect(figure).toMatch(/\{#if !captionShared\}/);
    expect(figure).not.toContain('compact');
    // And the language is required, so no caller can reach a bandless one.
    expect(figure).toMatch(/\n {4}language: BandLanguage;/);
