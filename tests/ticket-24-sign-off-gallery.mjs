/* Renders for ticket 24 (phase 12 final audit U15), for sign-off.

   Only what the ticket changed: the Ignored words screen's header, whose
   subtitle now says where a word comes from (before: what the reading
   skips; after: that plus how to add one).
   ...
   Before and after both, since the ticket changed an existing screen
   rather than adding a new one. Run `--tag before` from a checkout at the
   ticket's own merge base as its cwd - vite's `preview({ root })` serves
   the *cwd's* build rather than root's - so this script has to be copied
   there and run with that checkout as the working directory:

     VITE_DEMO=1 npm run build   # in the before checkout
     node tests/ticket-24-sign-off-gallery.mjs --tag before --out /abs/dir

     VITE_DEMO=1 npm run build   # in this ticket's own checkout
     node tests/ticket-24-sign-off-gallery.mjs --tag after --out /abs/dir */
...
const outDir = resolve(flag('out', resolve('.claude/ticket-24-sign-off-shots')), tag);
const width = 390;
...
for (const theme of ['light', 'dark']) {
  ...
  // The Ignored words screen's header (title + subtitle).
  await settle('/settings/words');
  await page.waitForSelector('[data-screen-header]');
  await crop(`words-ignored-header-${theme}`, '[data-screen-header]');
}
