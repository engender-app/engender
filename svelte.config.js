...
/* SvelteKit's build id, which the offline-shell cache is keyed to and which is
   kept separate from the public version name. Its default is the time of the
   build, and it is folded into the entry chunk, so it changes every chunk
   hash: two builds of the same commit share no bytes.
...
const config = {
  ...
  kit: {
    ...
    /* The production CSP, written into the document as a meta policy.
       ...
       Why the *whole* set is here and not just script-src: the Capacitor
       shell serves its own origin from the APK and never sees an
    ...
    paths: {
      /* Root-absolute asset URLs, against SvelteKit's default of relative
         ...
         /settings/labs as at /. Both this app and Capacitor's
