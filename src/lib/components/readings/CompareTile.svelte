<script lang="ts">
  /* Compare's tile on the Look back door (phase 11 ticket 07). The screen
     is `/compare`, opened filled since redesign ticket 05 - the span as
     side A and the same-length stretch before it as side B, through the
     query compareStretch.ts mints. The tile's figure is that stretch's two
     dates, since the span's own are written under the rail already.
  ...
  import { compareStretchQuery, precedingWindow, stretchTooShortToCompare } from '$lib/data/compareStretch';
...
{#if !short}
  <ReadingTile
    ...
    href={compareStretchQuery(span, before)}
