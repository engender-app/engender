...
/** The picked file's own size on disk, checked before it is read into
    ...
    inflated ceiling once it does. Same human step as above. */
...
export interface ZipReader {
  ...
  /** One entry's bytes, or null when the archive has no such entry.
      Decompresses only that member, and only once its declared size, added
      to every size this reader has already accepted, still fits under the
      ceiling it was opened with - otherwise throws `ZipTooLargeError`
