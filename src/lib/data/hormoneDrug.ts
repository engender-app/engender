...
/** The hormones this app draws a curve of. These are also the analyte names
    ...
    curve's analyte is the drug it models, so the same string asks the
    allowlist for a conversion and names the drug. */
export const CURVE_DRUGS = ['estradiol', 'testosterone'] as const;

export type CurveDrug = (typeof CURVE_DRUGS)[number];

/** The unit each hormone's curve is drawn in - the unit its published
    parameters were published in, not a unit anyone chose. Both are on
    ADR-0026's allowlist, so a modelled value converts for display exactly
    the way one of the user's own results does. */
export const CURVE_UNITS: Record<CurveDrug, string> = {
  estradiol: 'pg/mL',
  testosterone: 'ng/dL'
};
