...
/** Every production file that reads this module - phase 10 redesign ticket
    ...
    is one of these eight, so a ninth reader appearing anywhere fails here
    ...
    `dayRows.ts` and `searchHitRows.ts` are the seventh and eighth, added by
    ticket 30: a search hit and a day row used to declare an icon and a route
    of their own for every screen the hub already named, an agreement the
    two files stated only in a comment. They now read a row's `icon` and
    `href` off `hubRow()` wherever a screen has one - the same reasoning
    `TodayEditor.svelte` already stood on - and keep their own words and
    local icons only where no hub row owns the screen at all. */
