...
export type DocumentFile = NormalizedPhoto | { pdfBytes: Uint8Array };
...
function withinCeiling<T extends DocumentFile>(content: T, byteLength: number): T {
...
export async function acceptDocumentFile(bytes: Uint8Array): Promise<DocumentFile> {
  if (isPdf(bytes)) return withinCeiling({ pdfBytes: bytes }, bytes.byteLength);
  ...
  return withinCeiling(photo, photo.full.byteLength);
}
