...
/** The shortest arc still visible as a segment. A share smaller than the
    gap would otherwise come out at zero length or negative, which is the
    small-share case this form has to survive: it is a real reading and it
    keeps its place on the ring. */
...
export function sliced(parts: Part[], restName: string, cap: number = MAX_SLICES): Slice[] {
  ...
  const share = (amount: number) => (amount / whole) * 100;
  if (ranked.length <= cap) {
    return ranked.map((p) => ({ ...p, share: share(p.amount), isRest: false }));
  }
  ...
  return [
    ...named.map((p) => ({ ...p, share: share(p.amount), isRest: false })),
    { key: 'rest', name: restName, amount: restAmount, share: share(restAmount), isRest: true }
  ];
}
...
export function arcs(shares: number[], circumference: number): Arc[] {
  ...
  return shares.map((sharePercent) => {
    ...
    const dash = length <= 0 ? 0 : single ? length : Math.max(length - ARC_GAP, MIN_ARC);
