...
episode** overall, regardless of which direction a person is on. That
episode's drug decides which bands may be drawn against it: the feminizing
four only where it names estradiol, the masculinizing four only where it
names testosterone, and none at all for a drug the app cannot classify - a
blocker, progesterone alone or free text no list matches. The gate is on the
